...
import PlanExerciseService from '../../services/planExercise';
import {
...
export default class PlanExerciseController {
  ...
  static async create(request: Request, response: Response) {
    ...
    if (!body.plan_id) {
      return response.badrequest({ errors: { name: 'Plan is required' } });
    }

    if (!body.exercise_id) {
      return response.badrequest({ errors: { name: 'Exercise is required' } });
    }

    if (!body.repetitions) {
      return response.badrequest({ errors: { name: 'Repetition is required' } });
    }

    if (!body.rest_between_sets) {
      return response.badrequest({ errors: { name: 'Rest between sets is required' } });
    }

    if (!body.sets) {
      return response.badrequest({ errors: { name: 'Sets are required' } });
    }

    if (!body.day) {
      return response.badrequest({ errors: { name: 'Day is required' } });
    }
